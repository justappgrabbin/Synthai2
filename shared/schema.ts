...
export interface CollaborationSession {
  ...
  fileVersions: Record<string, number>; // fileName -> version
  createdAt: string;
  lastActivity: string;
  isActive: boolean;
}
...
export type CollaborationEvent = 
  | { type: 'participant:join'; participant: CollaborationParticipant }
  | { type: 'participant:leave'; participantId: string }
  | { type: 'cursor:move'; cursor: CursorPosition }
  | { type: 'file:change'; change: FileChange }
  | { type: 'file:open'; fileName: string; participantId: string }
  | { type: 'session:sync'; session: CollaborationSession }
  | { type: 'ping'; participantId: string };
